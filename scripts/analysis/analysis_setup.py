...
def main():
    ...
    for asset_info in sector_attributes.itertuples():
        ...
        while rp_min < flood_return_periods[-1]:
            ...
            #                                 sort='False',
            #                                 ignore_index=True).fillna(0)
            # sector_losses_all.to_parquet(os.path.join(results_path,
            #                 f"{asset_sector}_{asset_info.asset_layer}_{flood_protection_name}_losses.parquet"),
            #                 index=False)
            # del sector_losses_all

            # sector_risks_all = pd.concat(sector_risks_all,
            #                                 axis=0,
            #                                 sort='False',
            #                                 ignore_index=True).fillna(0)
            # sector_risks_all.to_parquet(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_risks.parquet'),
            #                                         index=False)

            # del sector_risks_all

            # sector_npv_all = pd.concat(sector_npv_all,
            #                             axis=0,
            #                             sort='False',
            #                             ignore_index=True).fillna(0)

            # if flood_protection_name == "no_protection_rp": 
            #     sector_no_protection_npvs = sector_npv_all.copy()
            #     sector_no_protection_npvs.drop("no_protection_rp",axis=1,inplace=True)
            # else:
            #     sector_npv_all = rad.get_adaptation_benefits(sector_npv_all,
            #                                                 sector_no_protection_npvs,
            #                                                 [flood_protection_name] + adapt_cost_columns)


            # sector_npv_all.to_parquet(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_npvs.parquet'),
            #                                         index=False)

            # sector_npv_all.to_csv(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_npvs.csv'),
            #                                         index=False)    
            # del sector_npv_all

            # sector_ead_timeseries_all = pd.concat(sector_ead_timeseries_all,
            #                                         axis=0,
            #                                         sort='False',
            #                                         ignore_index=True).fillna(0)
            # sector_ead_timeseries_all.columns = sector_ead_timeseries_all.columns.astype(str)
            # sector_ead_timeseries_all.to_parquet(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_ead_timeseries.parquet'),
            #                                         index=False)
            # del sector_ead_timeseries_all

            # sector_eael_timeseries_all = pd.concat(sector_eael_timeseries_all,
            #                                         axis=0,
            #                                         sort='False',
            #                                         ignore_index=True).fillna(0)
            # sector_eael_timeseries_all.columns = sector_eael_timeseries_all.columns.astype(str)
            # sector_eael_timeseries_all.to_parquet(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_eael_timeseries.parquet'),
            #                                         index=False)
            # del sector_eael_timeseries_all

            # sector_ead_timeseries_discounted = pd.concat(sector_ead_timeseries_discounted,
            #                                         axis=0,
            #                                         sort='False',
            #                                         ignore_index=True).fillna(0)
            # sector_ead_timeseries_discounted.columns = sector_ead_timeseries_discounted.columns.astype(str)
            # sector_ead_timeseries_discounted.to_parquet(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_ead_timeseries_discounted.parquet'),
            #                                         index=False)
            # del sector_ead_timeseries_discounted

            # sector_eael_timeseries_discounted = pd.concat(sector_eael_timeseries_discounted,
            #                                         axis=0,
            #                                         sort='False',
            #                                         ignore_index=True).fillna(0)
            # sector_eael_timeseries_discounted.columns = sector_eael_timeseries_discounted.columns.astype(str)
            # sector_eael_timeseries_discounted.to_parquet(os.path.join(results_path,
            #                         f'{asset_sector}_{flood_protection_name}_eael_timeseries_discounted.parquet'),
            #                                         index=False)
            # del sector_eael_timeseries_discounted
